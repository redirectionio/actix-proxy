...
impl ForwardedHeaderStrategy {
    pub fn apply(&self, headers: &mut HeaderMap, by: &str, client_ip: &str, host: String, proto: Scheme) {
        match self {
            ...
            Self::Auto => {
                if headers.contains_key(header::X_FORWARDED_FOR) {
                    if headers.contains_key(header::X_FORWARDED_PROTO)
                        || headers.contains_key(header::X_FORWARDED_HOST)
    ...
    fn apply_legacy(headers: &mut HeaderMap, by: &str, client_ip: &str, host: String, proto: Scheme) {
        ...
        if !forwarded_for_existing_value.is_empty() {
            ...
            headers.insert("x-forwarded-for".parse().unwrap(), forwarded_for_existing_value.parse().unwrap());
        } else {
            headers.insert("x-forwarded-for".parse().unwrap(), client_ip.parse().unwrap());
        }

        headers.insert("x-forwarded-by".parse().unwrap(), by_value.parse().unwrap());

        if !headers.contains_key(header::X_FORWARDED_PROTO) {
            let proto_value = match headers.get(header::X_FORWARDED_PROTO) {
                Some(value) => format!("{}, {}", value.to_str().unwrap(), proto.as_str()),
                None => proto.to_string(),
            };

            headers.insert(header::X_FORWARDED_PROTO, proto_value.parse().unwrap());
        }

        if !headers.contains_key(header::X_FORWARDED_HOST) {
...
mod tests {
    ...
    fn test_forward_legacy() {
        ...
        headers.insert(actix_web::http::header::X_FORWARDED_FOR, "127.0.0.1".parse().unwrap());
        headers.insert(actix_web::http::header::X_FORWARDED_PROTO, "https".parse().unwrap());
        headers.insert(actix_web::http::header::X_FORWARDED_HOST, "localhost".parse().unwrap());

        let strategy = ForwardedHeaderStrategy::Legacy;
        strategy.apply(&mut headers, "by", "192.168.0.1", "test.com".to_string(), Scheme::HTTP);

        assert_eq!(
            headers.get(actix_web::http::header::X_FORWARDED_FOR).unwrap().to_str().unwrap(),
            "127.0.0.1, 192.168.0.1"
        );
        assert_eq!(
            headers.get(actix_web::http::header::X_FORWARDED_PROTO).unwrap().to_str().unwrap(),
            "https"
        );
        assert_eq!(
            headers.get(actix_web::http::header::X_FORWARDED_HOST).unwrap().to_str().unwrap(),
            "localhost"
        );
    }
    ...
    fn test_forward_none() {
        ...
        assert_eq!(headers.get(actix_web::http::header::FORWARDED), None);
        assert_eq!(headers.get(actix_web::http::header::X_FORWARDED_FOR), None);
        assert_eq!(headers.get(actix_web::http::header::X_FORWARDED_PROTO), None);
        assert_eq!(headers.get(actix_web::http::header::X_FORWARDED_HOST), None);
    }
    ...
    fn test_forward_rfc7239() {
        ...
        assert_eq!(
            headers.get(actix_web::http::header::FORWARDED).unwrap().to_str().unwrap(),
    ...
    fn test_forward_legacy_multiple() {
        ...
        headers.append(actix_web::http::header::X_FORWARDED_FOR, "127.0.0.1".parse().unwrap());
        headers.append(actix_web::http::header::X_FORWARDED_FOR, "127.0.0.2".parse().unwrap());
        headers.append(actix_web::http::header::X_FORWARDED_PROTO, "https".parse().unwrap());
        headers.append(actix_web::http::header::X_FORWARDED_HOST, "localhost".parse().unwrap());
    ...
    fn test_forward_legacy_no_proto() {
        ...
        assert!(!headers.contains_key(actix_web::http::header::FORWARDED));
    }
